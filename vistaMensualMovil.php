...
$sql = "SELECT eventos.id, eventos.nombre as nombre_evento, eventos.fecha, eventos.anotaciones, asignaturas.abreviatura as abreviatura_asignatura FROM eventos INNER JOIN asignaturas ON eventos.asignatura_id = asignaturas.id ORDER BY eventos.fecha ASC";
...
while ($row = $result->fetch_assoc()) {
	$eventos[$row["fecha"]][] = [
		...
		'abreviatura' => $row["abreviatura_asignatura"],
	];
}
...
<html lang="es">
	...
	<body>
		<div class="cabecera">
			...
			<a class="tituloDerecha" href="/vistaMovil.php">Vista listado</a>
		</div>
		<div class="cuerpo">
			<h2 id="nombreMes"><?php echo nombreMes($calendar->modify('-1 month')->format('F')) . ' ' . $calendar->format('Y'); ?></h2>

			<a href="?fecha=<?php echo $nextMonth; ?>" class="nav-button">&gt;</a>
			<a href="/vistaMensualMovil.php" class="nav-button">Mes actual</a>
			<a href="?fecha=<?php echo $prevMonth; ?>" class="nav-button">&lt;</a>
				...
				for ($day = $primerDia; $day <= $ultimoDia; $day->modify('+1 day')) {
					...
					if (isset($eventos[$fechaActual])) {
						...
						foreach ($eventos[$fechaActual] as $evento) {
							echo "<div class=\"asignatura\">" . $evento['abreviatura'] . "</div>";
		...
		<script>
			...
			if (window.innerWidth > 1000) { // Puedes ajustar el valor de 768 según tus necesidades
				window.location.href = "https://calendario.davidmartos.dev/vistaMensual.php"; // Reemplaza con la URL a la que deseas redirigir
			}
		</script>
	</body>
</html>